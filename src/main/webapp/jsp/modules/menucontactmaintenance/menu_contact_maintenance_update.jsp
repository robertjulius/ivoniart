...
<body>
	...
	<s:form action="/modules/menucontactmaintenance/executeUpdate.action" theme="simple">
			...
			<tr>
				<td>
					<table>
						...
						<s:textfield key="resource.email" name="newEmail" theme="xhtml" size="30px" />
					</table>
				</td>
			</tr>
			<tr>
					...
					<table>
						...
						<tr>
							<td><sjr:tinymce key="resource.content" id="richtextTinymceEditor" name="newContent"
									height="150" width="500" editorTheme="simple" />
							</td>
						</tr>
					</table>
				</td>
					...
					<table>
						<tr>
							<td><input type="button" value="<s:text name="resource.cancel"/>"
								onclick="if (!confirmCancel()) {return;} $(this).closest('form').attr('action', '<%=request.getContextPath()%>/modules/menucontactmaintenance/detail.action'); $(this).closest('form').submit();" /></td>
