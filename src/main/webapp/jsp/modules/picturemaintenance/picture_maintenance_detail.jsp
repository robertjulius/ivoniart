			...
			<tr>
				<td>
					<table class="grid">
						<thead>
							<tr>
								<td colspan="2"><s:text name="resource.pictureInformation" /></td>
							</tr>
						</thead>
						<tbody>
							<tr>
							...
							<tr>
								...
								<td align="left" width="300px"><s:property value="old.comment" /></td>
							</tr>
						</tbody>
					</table>
				</td>
					...
					<table>
						<tr>
							<td><input type="button" value="<s:text name="resource.back"/>"
								onclick="$(this).closest('form').attr('action', '<%=request.getContextPath()%>/modules/picturemaintenance/searchResult.action'); $(this).closest('form').submit();" /></td>
							<td><input type="button" value="<s:text name="resource.edit"/>"
								onclick="$(this).closest('form').attr('action', '<%=request.getContextPath()%>/modules/picturemaintenance/prepareUpdate.action'); $(this).closest('form').submit();" /></td>
							<td><input type="button" value="<s:text name="resource.delete"/>"
								onclick="$(this).closest('form').attr('action', '<%=request.getContextPath()%>/modules/picturemaintenance/executeDelete.action'); if (confirmAction()) {$(this).closest('form').submit();}" /></td>
						</tr>
					</table>
				</td>
			</tr>
